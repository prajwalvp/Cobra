...
class Search(object):
    ...
    def log_probability(self, params):
        ...
        transformed_params = np.array([
            (self.Cand.pmax[i] - self.Cand.pmin[i]) * (params[i] % (2 * np.pi) if self.Cand.wrapped[i] else params[i]) + self.Cand.pmin[i]
    ...
    def sample_emcee(self, nwalkers=1000, nsteps=1000, resume=False, doplot=False, sample=True):
        '''
