...
class DatFile(object):
    ...
    def setupDat(self):
        ...
        if (self.doBary == True):
            ...
            self.getBaryCorrs()
            interpBCorrs = interp.griddata(
                24*60*60*(self.BCorrs[0]-np.min(self.BCorrs[0])), self.BCorrs[1], self.BaseTime)
...
# self.Imag = gpuarray.to_gpu(np.float64(gpu_fftData.imag[1:-1].get()))
    ...
    def getBaryCorrs(self):
        ...
        for i in range(len(x)):
            newtim.write("barytim "+str(self.LowChan) +
                         " "+str(x[i])+" 0.1 pks\n")
    ...
    def CalcNoise(self, cut=False, mode=0):
        ...
        step = 100
        blocks = np.ceil(1.0*self.FSamps/step).astype(int)
