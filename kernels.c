...
extern "C" __global__ void AddAcceleration(double *a, double *orig,
                                           double accel, double period,
...
AddInterpCircBinary(double *a, double *orig, double *InterpCosBinary,
                    double *InterpSinBinary, double BinaryPeriod,
                    double BinaryPhase, double BinaryAmp, double phase,
                    double period, double width, double blin, double eta,
                    double etaB, double Heta2B) {
  const int i = blockDim.x * blockIdx.x + threadIdx.x;
...
AddInterpEccBinary(double *a, double *orig, double *InterpCosBinary,
                   ...
                   double BinarySinW, double Ecc, double phase, double period,
                   double width, double blin, double Alpha, double Beta) {
  const int i = blockDim.x * blockIdx.x + threadIdx.x;
  ...
  int LowBin = floor(BPhase);
  int HighBin = LowBin + 1;
  double CosBinarySignal =
      InterpCosBinary[LowBin] +
      (InterpCosBinary[HighBin] - InterpCosBinary[LowBin]) * (BPhase - LowBin);
  double SinBinarySignal =
      InterpSinBinary[LowBin] +
      (InterpSinBinary[HighBin] - InterpSinBinary[LowBin]) * (BPhase - LowBin);
  ...
  double BinarySignal = Dre * (1 - eta * Drep +
                               eta * eta *
                                   (Drep * Drep + 0.5 * Dre * Drepp -
                                    0.5 * Ecc * SinBinarySignal * Dre * Drep /
                                        (1 - Ecc * CosBinarySignal)));
...
extern "C" __global__ void
AddInterpGRBinary(double *a, double *orig, double *InterpCosBinary,
                  double *InterpSinBinary, double *InterpTrueAnomaly,
                  double BinaryPeriod, double BinaryPhase, double BinaryAmp,
                  double BinaryOmega, double Ecc, double M2, double OMDot,
                  double SINI, double Gamma, double PBDot, double SqEcc_th,
                  double Ecc_r, double arr, double ar, double phase,
                  double period, double width, double blin, double pepoch) {

  const int i = blockDim.x * blockIdx.x + threadIdx.x;

  // double BPhase = (orig[i]/BinaryPeriod)*(1.0 -
  // 0.5*PBDot*(orig[i]/BinaryPeriod)) + BinaryPhase;
  double BPhase = (orig[i] / BinaryPeriod + BinaryPhase) *
                  (1.0 - 0.5 * PBDot * (orig[i] / BinaryPeriod + BinaryPhase));
  ...
  double SinBinarySignal =
      InterpSinBinary[LowBin] +
      (InterpSinBinary[HighBin] - InterpSinBinary[LowBin]) * (BPhase - LowBin);
  double TrueAnomaly = InterpTrueAnomaly[LowBin] + (InterpTrueAnomaly[HighBin] -
                                                    InterpTrueAnomaly[LowBin]) *
                                                       (BPhase - LowBin);
  ...
  double brace = onemecu - SINI * (SinOmega * cume +
                                   SqEcc_th * CosOmega * SinBinarySignal);
  ...
  double BinarySignal =
      dre * (1 - anhat * drep +
             (anhat * anhat) *
                 (drep * drep + 0.5 * dre * drepp -
                  0.5 * Ecc * SinBinarySignal * dre * drep / onemecu)) +
      ds;
...
extern "C" __global__ void
AddInterpCircBinary2(double *a, double *orig, double *InterpCosBinary,
                     double *InterpSinBinary, double BinaryPeriod,
                     double BinaryCosAmp, double BinarySinAmp, double blin,
                     double pepoch) {
  const int i = blockDim.x * blockIdx.x + threadIdx.x;
  ...
  int HighBin = LowBin + 1;
  double BinaryCosSignal =
      BinaryCosAmp * InterpCosBinary[LowBin] +
      (InterpCosBinary[HighBin] - InterpCosBinary[LowBin]) * (BPhase - LowBin);
  double BinarySinSignal =
      BinarySinAmp * InterpSinBinary[LowBin] +
      (InterpSinBinary[HighBin] - InterpSinBinary[LowBin]) * (BPhase - LowBin);

  a[i] =
      orig[i] + BinaryCosSignal + BinarySinSignal - blin * (orig[i] - pepoch);
}

extern "C" __global__ void AddCircBinary(double *a, double *orig,
                                         double BinaryAmp, double BinaryPeriod,
                                         double BinaryPhase, double phase,
                                         double blin, double pepoch) {
  const int i = blockDim.x * blockIdx.x + threadIdx.x;

  a[i] = orig[i] +
         BinaryAmp * cos(2 * M_PI * orig[i] / BinaryPeriod + BinaryPhase) -
...
extern "C" __global__ void Scatter(double *real, double *imag, double TimeScale,
                                   double *samplefreqs) {
  const int i = blockDim.x * blockIdx.x + threadIdx.x;
  ...
  double RConv =
      1.0 / (samplefreqs[i] * samplefreqs[i] * TimeScale * TimeScale + 1);
  double IConv =
      -samplefreqs[i] * TimeScale /
      (samplefreqs[i] * samplefreqs[i] * TimeScale * TimeScale +
       1); // NB Timescale = Tau/(pow(((chanfreq, 4)/pow(10.0, 9.0*4.0));
